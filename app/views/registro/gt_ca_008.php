...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        if (!validar_numeric($nivel, 0) || !validar_numeric($presion, 0)) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Medición de tanque registrada con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close(); $stmt2->close();
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'error', 'message' => 'Error al registrar las mediciones del tanque.']);
                        exit;
                    }
                }
...
$registros = mysqli_query($con, "
    SELECT rd.id, rd.fecha, i.nombre as planta, rd.observaciones_generales,
           m1.valor_medido as nivel, m2.valor_medido as presion
    FROM registro_diario rd
    JOIN instalacion i ON rd.id_instalacion = i.id
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 13
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 14
...
<div class="contenedor">
    <div class="page-hdr">
        <h2><i class="fas fa-water"></i> Niveles y Presión <span class="badge-gt">GT-CA-008</span></h2>
        <p class="subtitle">Control de Niveles de Agua y Presión en Tanques</p>
    </div>
    <?php if ($mensaje == "exito"): ?>
        <div class="alerta-exito"><i class="fas fa-check-circle"></i> ¡Niveles registrados con éxito!</div>
    ...
    <div class="gtca-wrapper">
        ...
        <div class="tabla-wrapper">
            <table class="tabla">
                <thead><tr><th>Fecha</th><th>Planta</th><th>Tanque</th><th>Nivel (m)</th><th>Presión (PSI)</th></tr></thead>
                <tbody>
                    <?php while ($r = mysqli_fetch_assoc($registros)): ?>
                    <tr>
                        <td><?php echo date('d/m/Y', strtotime($r['fecha'])); ?></td>
                        <td><strong><?php echo hsc($r['planta']); ?></strong></td>
                        <td><?php echo hsc(str_replace('Control de Tanque: ', '', $r['observaciones_generales'] ?? '')); ?></td>
                        <td><?php echo number_format((float)($r['nivel'] ?? 0), 2); ?></td>
                        <td><?php echo number_format((float)($r['presion'] ?? 0), 1); ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_008">
            <?php echo csrf_field(); ?>
            <div class="modal-header">
                <h3><i class="fas fa-water"></i> <span>Nueva Medición de Nivel y Presión</span></h3>
            ...
            <div class="modal-body">
                ...
                <div class="modal-grid">
                    <div class="modal-campo"><label>Nivel de Agua (Metros)</label><input type="number" step="0.01" name="nivel" required placeholder="0.00"></div>
                    <div class="modal-campo"><label>Presión de Salida (PSI)</label><input type="number" step="0.1" name="presion" required placeholder="0.0"></div>
                </div>
            </div>
