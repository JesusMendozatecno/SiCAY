...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        $combustible = $_POST['combustible']; $bateria = $_POST['bateria'];
        $temp = $_POST['temp']; $novedad = trim($_POST['novedad'] ?? '');
        $id_user = (int) $_SESSION['id_usuario'];
        $fecha = date('Y-m-d'); $hora = date('H:i:s');
        if (!validar_numeric($combustible, 0) || !validar_numeric($bateria, 0) || !validar_numeric($temp, 0)) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute() && $stmt3->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Chequeo de generador registrado con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close(); $stmt2->close(); $stmt3->close();
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'error', 'message' => 'Error al registrar las mediciones del generador.']);
                        exit;
                    }
                }
...
$registros = mysqli_query($con, "
    SELECT rd.id, rd.fecha, i.nombre as planta, rd.observaciones_generales,
           m1.valor_medido as combustible, m2.valor_medido as bateria, m3.valor_medido as temp
    FROM registro_diario rd
    JOIN instalacion i ON rd.id_instalacion = i.id
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 22
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 23
    JOIN medicion_horaria m3 ON m3.id_registro_diario = rd.id AND m3.id_parametro = 24
...
<div class="contenedor">
    <div class="page-hdr">
        <h2><i class="fas fa-plug"></i> Grupo Electrógeno <span class="badge-gt">GT-CA-012</span></h2>
        <p class="subtitle">Control de Generador Eléctrico — Combustible, Batería y Temperatura</p>
    </div>
    <?php if ($mensaje == "exito"): ?>
        <div class="alerta-exito"><i class="fas fa-check-circle"></i> ¡Reporte guardado con éxito!</div>
    ...
    <div class="gtca-wrapper">
        <div class="tabla-toolbar">
            ...
            <button class="tabla-btn-agregar" data-modal-open="modalRegistro"><i class="fas fa-plus"></i> Nuevo Reporte</button>
        </div>
        <div class="tabla-wrapper">
            <table class="tabla">
                <thead><tr><th>Fecha</th><th>Planta</th><th>Combustible (%)</th><th>Batería (Vdc)</th><th>Temp. Motor (°C)</th><th>Observaciones</th></tr></thead>
                <tbody>
                    <?php while ($r = mysqli_fetch_assoc($registros)): ?>
                    <tr>
                        <td><?php echo date('d/m/Y', strtotime($r['fecha'])); ?></td>
                        <td><strong><?php echo hsc($r['planta']); ?></strong></td>
                        <td><?php echo number_format((float)($r['combustible'] ?? 0), 0); ?>%</td>
                        <td><?php echo number_format((float)($r['bateria'] ?? 0), 1); ?></td>
                        <td><?php echo number_format((float)($r['temp'] ?? 0), 1); ?></td>
                        <td style="font-size:13px;"><?php echo hsc(str_replace('Chequeo Generador: ', '', $r['observaciones_generales'] ?? '')); ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_012">
            <?php echo csrf_field(); ?>
            <div class="modal-header">
                <h3><i class="fas fa-plug"></i> <span>Nuevo Reporte de Generador</span></h3>
            ...
            <div class="modal-body">
                ...
                <div class="modal-grid">
                    <div class="modal-campo"><label>Combustible (%)</label><input type="number" name="combustible" required placeholder="Ej: 80"></div>
                    <div class="modal-campo"><label>Voltaje Batería (Vdc)</label><input type="number" step="0.1" name="bateria" required placeholder="Ej: 13.5"></div>
                    <div class="modal-campo"><label>Temperatura Motor (°C)</label><input type="number" step="0.1" name="temp" required placeholder="Ej: 75"></div>
                </div>
                <div class="modal-campo">
