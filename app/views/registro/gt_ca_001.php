...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        $fecha = date('Y-m-d');
        $hora = date('H:i:s');
        if (!validar_numeric($cloro) || !validar_numeric($ph)) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Medición de Cloro y pH registrada con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close();
...
$registros = mysqli_query($con, "
    ...
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 1
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 3
            ...
            <table class="tabla">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format((float)($r['cloro'] ?? 0), 2); ?></td>
                        <td><?php echo number_format((float)($r['ph'] ?? 0), 2); ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_001">
            <?php echo csrf_field(); ?>
