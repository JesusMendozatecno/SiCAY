...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        if (!in_array($luz, [0,1]) || !in_array($limpieza, [0,1]) || !in_array($cerca, [0,1]) || !in_array($fugas, [0,1])) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
                echo json_encode(['status' => 'error', 'message' => 'Valores de inspección inválidos.']);
                exit;
            }
        } else {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute() && $stmt3->execute() && $stmt4->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Inspección física registrada con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close(); $stmt2->close(); $stmt3->close(); $stmt4->close();
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'error', 'message' => 'Error al registrar la inspección.']);
                        exit;
                    }
                }
...
$registros = mysqli_query($con, "
    ...
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 18
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 19
    JOIN medicion_horaria m3 ON m3.id_registro_diario = rd.id AND m3.id_parametro = 20
    JOIN medicion_horaria m4 ON m4.id_registro_diario = rd.id AND m4.id_parametro = 21
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_011">
            <?php echo csrf_field(); ?>
