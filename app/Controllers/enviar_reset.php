...
if ($result->num_rows > 0) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'jesus17otaku@gmail.com';
        $mail->Password   = 'Lobo118*';
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;
        $mail->setFrom('jesus17otaku@gmail.com', 'SICAY - Hidroven Yaracuy');
        $mail->addAddress($email);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = 'Recuperacion de contrasena - SICAY';
        $mail->isHTML(true);
        $mail->Body = "
        <div style='font-family:Arial;max-width:500px;margin:auto;padding:30px;background:#f4f7fc;border-radius:15px;'>
            ...
            <div style='background:white;padding:25px;border-radius:12px;'>
                <h3 style='color:#123C69;margin-top:0;'>Recuperacion de contrasena</h3>
                <p style='color:#555;'>Haz clic en el siguiente boton para restablecer tu contrasena:</p>
                <div style='text-align:center;margin:25px 0;'>
                    <a href='$link' style='background:#1F6AE1;color:white;padding:14px 30px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block;'>Restablecer Contrasena</a>
                </div>
                <p style='color:#999;font-size:0.85rem;'>Este enlace expirara en 1 hora. Si no solicitaste este cambio, ignora este correo.</p>
            </div>
