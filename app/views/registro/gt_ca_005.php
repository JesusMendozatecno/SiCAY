...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        if (!in_array($estado, $estados_validos)) {
            ...
            if ($is_ajax) {
                header('Content-Type: application/json');
                echo json_encode(['status' => 'error', 'message' => 'Estado del cilindro inválido.']);
                exit;
            }
        } elseif (!validar_numeric($peso, 0) || !validar_numeric($presion, 0)) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Control de cilindro registrado con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close();
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'error', 'message' => 'Error al registrar las mediciones del cilindro.']);
                        exit;
                    }
                }
            } else {
...
$registros = mysqli_query($con, "
    ...
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 6
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 7
            ...
            <table class="tabla">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format((float)($r['peso'] ?? 0), 1); ?></td>
                        <td><?php echo number_format((float)($r['presion'] ?? 0), 0); ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_005">
            <?php echo csrf_field(); ?>
