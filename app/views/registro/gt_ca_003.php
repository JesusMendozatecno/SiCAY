...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($faltantes)) {
        ...
        if (!in_array($etapa, $etapas_validas)) {
            ...
            if ($is_ajax) {
                header('Content-Type: application/json');
                echo json_encode(['status' => 'error', 'message' => 'Etapa del proceso inválida.']);
                exit;
            }
        } elseif (!validar_numeric($turbiedad, 0) || !validar_numeric($color, 0)) {
            $mensaje = "error";
            if ($is_ajax) {
                header('Content-Type: application/json');
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute() && $stmt2->execute()) {
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'success', 'message' => 'Análisis de Turbiedad y Color registrado con éxito.']);
                        exit;
                    }
                } else {
                    $stmt1->close();
                    ...
                    if ($is_ajax) {
                        header('Content-Type: application/json');
                        echo json_encode(['status' => 'error', 'message' => 'Error al registrar las mediciones.']);
                        exit;
                    }
                }
            } else {
...
$registros = mysqli_query($con, "
    ...
    JOIN medicion_horaria m1 ON m1.id_registro_diario = rd.id AND m1.id_parametro = 4
    JOIN medicion_horaria m2 ON m2.id_registro_diario = rd.id AND m2.id_parametro = 5
            ...
            <table class="tabla">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format((float)($r['turbiedad'] ?? 0), 2); ?></td>
                        <td><?php echo number_format((float)($r['color'] ?? 0), 2); ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
...
<div class="modal-overlay" id="modalRegistro">
    <div class="modal-contenido">
        <form method="POST" data-route="gt_ca_003">
            <?php echo csrf_field(); ?>
